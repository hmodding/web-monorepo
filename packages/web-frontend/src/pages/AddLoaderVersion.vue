<template>
  <div class="container">
    <section class="my-5 mx-1">
      <div class="card mb-3">
        <div class="card-body">
          <h5 class="card-title">Add loader version</h5>
          <form @submit.prevent="onSubmit">
            <api-provided-form
              v-if="ready"
              form-name="addLoaderVersion"
              :data="data"
              @change="onChange"
            />
            <hr />
...
export default defineComponent({
  ...
  beforeRouteEnter() {
    setDocumentTitle('Add loader version');
  },
  methods: {
    async onSubmit(): Promise<void> {
      if (!this.loadig) {
        ...
        this.loading = true;
        const loaderVersion = await api.addLoaderVersion(this.data);

        if (!!loaderVersion) {
          this.hasUnsavedChanges = false;
          await this.$router.push({ name: 'home' });
          toaster.success(
            `LoaderVersion <b>"${loaderVersion.rmlVersion}"</b> was added!`,
          );
        } else {
